...
export class TournamentCommand extends Subcommand {
					...
					builder
						...
						.addStringOption((option) =>
							option
								.setName("name")
								.setDescription(
									"The name of the tournament. (Max. 64 characters)",
								)
								.setRequired(true)
								.setMaxLength(64),
						)
	...
	public async chatInputRunCreate(
		...
		const [staffRole, mappoolerRole, refereeRole, playerRole] =
			await this.getTournamentRoles(interaction);
		...
		const [
			...
			scheduleChannel,
			playerChannel,
		] = await this.getTournamentChannels(
			interaction,
			staffRole,
			mappoolerRole,
			refereeRole,
	 ...
	 */
	private async getTournamentRoles(
		interaction: Subcommand.ChatInputCommandInteraction,
	) {
	 ...
	 */
	private async getTournamentChannels(
		interaction: Subcommand.ChatInputCommandInteraction,
		staffRole: Role,
		mappoolerRole: Role,
		refereeRole: Role,
